...
# Internet Interface
INET_IFACE="eth0"
INET_ADDRESS=$( /sbin/ifconfig eth0 | grep 'inet addr' | cut -d':' -f2 | cut -d' ' -f1 )
...
echo "Process SSH chain ..."
$IPT -N SSHATTACK
$IPT -A SSHATTACK -j LOG --log-prefix "SSH login blocked for target IP, too many connection attempts. " --log-level 7
$IPT -A SSHATTACK -j DROP

# Block an IP for N seconds when more than 9 connection attempts are made
$IPT -A INPUT -i $INET_IFACE -p tcp -m state --dport 22 --state NEW -m recent --set
$IPT -A INPUT -i $INET_IFACE -p tcp -m state --dport 22 --state NEW -m recent --update --seconds 240 --hitcount 10 -j SSHATTACK
